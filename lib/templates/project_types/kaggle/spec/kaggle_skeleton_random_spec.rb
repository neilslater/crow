...
describe KaggleSkeleton do
  let(:scalar_seed_examples) do
    [
  ...
  let(:array_seed_examples) do
    [
      ...
      [[7685, 7231, 2321, 2322, 75],
       [0.024745, 0.744554, 0.867087, 0.027255, 0.500870, 0.080460, 0.808293, 0.595685, 0.960167]],
      [[2013], [0.925177, 0.328296, 0.551907, 0.356516, 0.901791, 0.304950, 0.400617, 0.839359, 0.255680]]
    ]
  end

  def expect_random_sequence(seed_method, seed, expected_results)
    described_class.public_send(seed_method, seed)
    got_results = Array.new(expected_results.length) { described_class.rand }
    expected_matchers = expected_results.map { |expected| be_within(1e-6).of(expected) }
    expect(got_results).to match expected_matchers
  end
  ...
  describe 'random number generator' do
    it 'does not use the default mt.c seed when loaded' do
      ...
      script_output = `ruby -Ilib -e "require 'kaggle_skeleton'; puts KaggleSkeleton.rand"`
      got_num = script_output.chomp.to_f
      expect(got_num).not_to be_within(1e-8).of 0.3147237002849579
    end

    it 'generates numbers consistently when seeded' do
      scalar_seed_examples.each do |seed, expected_results|
        expect_random_sequence(:srand, seed, expected_results)
      end
    end

    it 'can accept seeds which are arrays of integers' do
      array_seed_examples.each do |seed, expected_results|
        expect_random_sequence(:srand_array, seed, expected_results)
      end
    end
  end
end
